...
- [x] 0.4.1 too: Show solution one move at a time (Settings > Difficulty; `reveal_next` in session and run; Show next move in the run page and the puzzle windows; the life goes once). Tests, screenshots, the draft's zip + APK + PDF and notes replaced (2026-09-24)
- [x] Release v0.4.1 published as Latest after John's Windows check (2026-09-24)
...
- 2026-09-23: a release meant for sharing gets a full feature page with screenshots as its notes, the screenshots committed under docs/screenshots and regenerated by tools/release_shots.py (offscreen, real database, seeded runs) - John posts the release link, so the page has to sell the app on its own; committing the images keeps the release page and the README pointing at one place
